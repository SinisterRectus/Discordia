...
- All programmatic mentions of "server" or "Server" have been changed to "guild" or "Guild", though the words may be used interchangeably in discussions.
...
- A new `client` directory now houses all modules and classes that are closely related to client operations.
  - The `Client` class was overhauled, though it maintains most of its original behavior:
	...
	- `Client:request` was moved to `API:request`
	...
	- Much of the internal websocket handling was moved into the `Socket` class
  - A new `API` class used to call REST methods is introduced
	...
	- Rate limiting is overhauled by using the new `RateLimiter` class
