...
method('run', run, 'token', "Connects to a Discord gateway using a valid Discord token and starts the main program loop(s).", 'HTTP and WS')
method('stop', stop, 'shouldExit', "Disconnects from the Discord gateway and optionally exits the process.", 'WS')

method('listVoiceRegions', listVoiceRegions, nil, "Returns a table of voice regions.", 'HTTP')
