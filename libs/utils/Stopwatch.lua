...
function Stopwatch:getTime()
	local ns = (self._final or uv.hrtime()) - self._initial
	return Time.fromMicroseconds(ns / NS_PER_US)
end

function Stopwatch:start()
	if not self._final then return end
	self._initial = self._initial + uv.hrtime() - self._final
	self._final = nil
end

function Stopwatch:stop()
	if self._final then return end
	self._final = uv.hrtime()
end

function Stopwatch:reset()
	self._initial = self._final or uv.hrtime()
end
